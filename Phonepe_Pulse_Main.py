...
icon = Image.open(r"D:\Guvi\Capstone\GUVI-CAPSTONE-PROJECTS\Phonepe_Pulse_Data_Visualization\Data\ICN.png")
st.set_page_config(page_title= "Phonepe Pulse Data Visualization | By Santhosh Kumar",
                   page_icon= icon,
                   layout= "wide",
                   initial_sidebar_state= "expanded",
                   menu_items={'About': """# This dashboard app is created by *Santhosh Kumar*!
...
# Top Charts - USERS          
    if Type == "Users":
        ...
        with col2:
            st.markdown("### :violet[District]")
            mycursor.execute(f"select district, sum(Registered_User) as Total_Users, sum(App_Opens) as Total_App_opens from map_user where year = {Year} and quarter = {Quarter} group by district order by Total_Users desc limit 10")
        ...
        with col4:
            st.markdown("### :violet[State]")
            mycursor.execute(f"select state, sum(Registered_user) as Total_Users, sum(App_Opens) as Total_App_opens from map_user where year = {Year} and quarter = {Quarter} group by state order by Total_Users desc limit 10")
            df = pd.DataFrame(mycursor.fetchall(), columns=['State', 'Total_Users','Total_App_opens'])
...
# EXPLORE DATA - TRANSACTIONS
    if Type == "Transactions":
        ...
        with col1:
            ...
            df2 = pd.read_csv(r'D:\Guvi\Capstone\GUVI-CAPSTONE-PROJECTS\Phonepe_Pulse_Data_Visualization\Data\Statenames.csv')
            df1.State = df2
...
# EXPLORE DATA - USERS      
    if Type == "Users":
        ...
        mycursor.execute(f"select state, sum(Registered_User) as Total_Users, sum(App_Opens) as Total_App_opens from map_user where year = {Year} and quarter = {Quarter} group by state order by state")
        ...
        mycursor.execute(f"select State,year,quarter,District,sum(Registered_user) as Total_Users, sum(App_Opens) as Total_App_opens from map_user where year = {Year} and quarter = {Quarter} and state = '{selected_state}' group by State, District,year,quarter order by state,district")
        
        df = pd.DataFrame(mycursor.fetchall(), columns=['State','year', 'quarter', 'District', 'Total_Users','Total_App_opens'])
...
if selected == "About":
    ...
    with col1:
        ...
        st.write("https://github.com/Santhosh-2131/Phonepe_Pulse_Project")
